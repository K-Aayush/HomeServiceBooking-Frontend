...
const Profile = () => {
  ...
  const updateProfileImage = async (file: File) => {
    try {
      const formData = new FormData();
      formData.append("requiterProfileImage", file);
    ...
    try {
      if (window.confirm("Are you sure you want to delete this Account?")) {
        const { data } = await axios.delete(
          `${backendUrl}/api/requiter/deleteRequiterData`,
          {
            headers: {
              Authorization: requiterToken,
            },
      ...
      <div className="flex flex-col justify-center px-6 py-16 space-y-6 md:mx-16">
        <div className="relative w-32 h-32">
          <label htmlFor="image">
            <img
              src={
                requiterData?.requiterProfileImage ||
            ...
            <div className="flex items-center w-full max-w-md gap-6 mt-2">
              {isEditing.name ? (
                ...
                <>
                  <Input
                    className="w-full"
                    placeholder={`${requiterData?.firstName || ""} ${
                      requiterData?.lastName || ""
                    }`}
                    disabled
                  />
                  <Button
